...
create table if not exists products (
  ...
  compat        jsonb  not null default '[]'::jsonb,  -- [{brand, models:[{name, stock, colors:[{name,hex,url,publicId,stock}]}]}] : stock PAR modèle/couleur
  colors        jsonb  not null default '[]'::jsonb,  -- [{name,hex,url,publicId,stock}] couleurs GLOBALES (repli si aucune marque)
  active        boolean not null default true,
